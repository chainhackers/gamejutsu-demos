...
import {
  TicTacToe, PlayerType as TicTacToePlayerType,
} from 'components/Games/Tic-Tac-Toe'
...
import {
  Checkers, PlayerType as CheckersPlayerType,
...
import {
  GameProposedEvent, GameProposedEventObject,
...
const Game: NextPage<IGamePageProps> = ({ gameType, version }) => {
  ...
  const [isResolveTimeOutAllowed, setIsResolveTimeOutAllowed] = useState<boolean>(
    false)
  const [isFinishTimeoutAllowed, setIsFinishTimeoutAllowed] = useState<boolean>(
    false)
  ...
  const [nextGameState, setNextGameState] = useState<IGameState<any, any> | null>(
    null)
  ...
  const [gameState, setGameState] = useState<IGameState<any, any>>(
    getInitialState())
  ...
  const runFinishGameHandler = async () => {
    ...
    const signature = await signMoveWithAddress(
      nextGameState.lastOpponentMove.gameMove, address)
    const signatures = [
      ...nextGameState.lastOpponentMove.signatures, signature]
    const lastOpponentMoveSignedByAll = new SignedGameMove(
      nextGameState.lastOpponentMove.gameMove, signatures)
    ...
    try {
      ...
      console.log('Завершение выполнения runFinishGameHandler',
        finishedGameResult)
      ...
      if (error.reason.includes('invalid game move')) {
        setFinishGameCheckResult(
          { winner: false, isDraw: false, cheatWin: true })
        router.push('/game-result')
        const newFinishResult = {
          winner: false, isDraw: false, cheatWin: true,
        }
  ...
  const initTimeoutHandler = async () => {
    ...
    try {
      const address = await (await getSigner()).getAddress()
      const signature = await signMoveWithAddress(
        gameState.lastOpponentMove.gameMove, address)
      const signatures = [...gameState.lastOpponentMove.signatures, signature]
      const lastOpponentMoveSignedByAll = new SignedGameMove(
        gameState.lastOpponentMove.gameMove, signatures)
      ...
      const initTimeoutResult = await initTimeout(await getArbiter(),
        [lastOpponentMoveSignedByAll, gameState.lastMove])
  ...
  const resolveTimeoutHandler = async () => {
    ...
    const resolveTimeoutResult = await resolveTimeout(await getArbiter(),
      gameState.lastMove)
  ...
  const createNewGameHandler = async (isPaid: boolean = false) => {
    let proposeGameResult: GameProposedEventObject = await gameApi.proposeGame(
      await getArbiter(), (await getRulesContract(gameType)).address, isPaid)
  ...
  const acceptGameHandler = async (
    acceptedGameId: number, stake: string): Promise<void> => {
    if (!account) {
    ...
    const acceptGameResult = await gameApi.acceptGame(await getArbiter(),
      BigNumber.from(acceptedGameId), stake)
  ...
  const finishTimeoutHandler = async () => {
    try {
      const finishedGameResult = await finalizeTimeout(await getArbiter(),
  ...
  const runDisputeHandler = async () => {
    ...
    const lastMoveOrLastOpponentMove = lastNonce > lastOpponentNonce &&
      gameState.lastMove !== null
    const lastMove = lastMoveOrLastOpponentMove
      ? gameState.lastMove
  ...
  async function processOneMessage (i: number) {
    ...
    if (lastMessage.messageType === 'ISignedGameMove') {
      ...
      console.log(
        `[gameType] processOneMessage signedMove: nonce: ${signedMove.gameMove.nonce}`,
        signedMove)
      ...
      const setNewGameState = async () => {
        try {
          ...
          const isValidMove = await isValidGameMove(contract,
            signedMove.gameMove)
          const isValid = await isValidSignedMove(contract, signedMove)
          
          const message = {
            contractAddress: contract.address,
            arguments: [{ signedMove }],
            validity: {
              isValidFront: frontIsValidMove,
              isValidMove,
              isValidSignedMove: isValid,
            },
          }
          const polygonMessageGameMove = [
          ...
          const polygonMessageSignedMove = [
            polygonMessageGameMove, signedMove.signatures]
          console.log('Requested move validation, contract message: ',
            JSON.stringify(polygonMessageSignedMove))
          console.log(
            `Requested move validation, nonce: ${signedMove.gameMove.nonce}\n`,
          ...
          const parsedStoredSignatures = !storedSignatures ? null : JSON.parse(
            storedSignatures)
          const signatureInfo = !parsedStoredSignatures
            ? null
            : parsedStoredSignatures[signedMove.signatures[0]]
          console.log('Requested move validation, signature data:',
            !parsedStoredSignatures ? 'not available' : signatureInfo)
          // END ******* DEBUG AND DEMO PURPOSE ONLY. TODO: DON'T DO THIS IN NON-DEMO APPS ******
          
          setMessageHistory((prev) => [
            ...prev, {
              nonce: String(signedMove.gameMove.nonce),
              message, // START ***** DEBUG AND DEMO PURPOSE ONLY. TODO: DON'T DO THIS IN NON-DEMO APPS ******
              signatureInfo: !signatureInfo ? 'not available' : signatureInfo, // END ******* DEBUG AND DEMO PURPOSE ONLY. TODO: DON'T DO THIS IN NON-DEMO APPS ******
              polygonMessageGameMove: JSON.stringify(polygonMessageGameMove),
              polygonMessageSignedMove: JSON.stringify(
                polygonMessageSignedMove),
            }])
          
          if (isValidMove !== isValid) {
            throw new Error(
              `isValidGameMove: ${isValidMove} is not equal to isValidSignedMove: ${isValid}`)
          }
          
          const previouseMove = i + 1 === lastMessages.length
            ? null
            : (lastMessages[i + 1].message as ISignedGameMove)
          const nextGameState = gameState.makeNewGameStateFromSignedMove(
            signedMove, isValid, isOpponentMove, previouseMove)
          ...
          if (winnerId !== null) {
            setFinishGameCheckResult({
              winner: playerIngameId === winnerId,
              isDraw: false,
              cheatWin: isValid,
            })
            setNextGameState(nextGameState)
          } else if (gameType === 'tic-tac-toe' && signedMove.gameMove.nonce ===
            8) {
            setFinishGameCheckResult(
              { winner: false, isDraw: true, cheatWin: isValid })
            setNextGameState(nextGameState)
          }
          return
        }
          ...
          console.error(
            '[gameType] processOneMessage messageType: ISignedGameMove error: ',
            count, error)
          if (count >= 10) return
  ...
  useEffect(() => {
    const runLastMessages = async (messages: IAnyMessage[]) => {
      ...
      setTimeout(() => runLastMessages(messages.slice(0, messages.length - 1)),
        500)
    }
  ...
  useEffect(() => {
    const isPlayerMoves = (gameType: TGameType, gameState: IGameState<any, any>,
      playerIngameId: 0 | 1) => {
      switch (gameType) {
      case 'checkers':
  ...
  useEffect(() => {
    if (gameState.lastOpponentMove?.gameMove.player === opponentAddress &&
      isInvalidMove) {
  ...
  useInterval(async () => {
    ...
    let players: [string, string] = await gameApi.getPlayers(
      await getArbiterRead(), BigNumber.from(gameId))
    ...
    if (!(players[0] === ZERO_ADDRESS && players[1] === ZERO_ADDRESS) &&
      ...
      throw new Error(
        `Player ${address} is not in game ${gameId}, players: ${players}`)
    }
  ...
  if (!!gameType && !!query && query?.prize === 'true') {
    return <SelectPrize gameId={gameId?.toString()}
                        createNewGameHandler={createNewGameHandler}/>
  }
      ...
      return (<div className={styles.container}>
        ...
        <RightPanel>
          <div style={{ position: 'absolute', right: '0' }}>
            <GetHistory history={lastMessages} messageHistory={messageHistory}
                        gameId={gameId}/>
          </div>
          <XMTPChatLog anyMessages={collectedMessages} isLoading={loading}/>
        </RightPanel>
        {gameType === 'checkers' && (<Disclaimer>
