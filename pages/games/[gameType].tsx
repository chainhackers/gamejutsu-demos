...
const Game: NextPage<IGamePageProps> = ({ gameType, version }) => {
  ...
  const { loading, collectedMessages, sendMessage, lastMessages, initClient, client, disconnect } = useConversation(opponentAddress!, gameId, true);
  ...
  const runFinishGameHandler = async () => {
    ...
    const lastOpponentMoveSignedByAll = new SignedGameMove(nextGameState.lastOpponentMove.gameMove, signatures);
  ...
  const initTimeoutHandler = async () => {
    ...
    try {
      ...
      const lastOpponentMoveSignedByAll = new SignedGameMove(gameState.lastOpponentMove.gameMove, signatures);
      console.log('lastOpponentMoveSignedByAll', lastOpponentMoveSignedByAll);

      const initTimeoutResult = await initTimeout(await getArbiter(), [lastOpponentMoveSignedByAll, gameState.lastMove]);
  ...
  const createNewGameHandler = async (isPaid: boolean = false) => {
    let proposeGameResult: GameProposedEventObject = await gameApi.proposeGame(await getArbiter(), (await getRulesContract(gameType)).address, isPaid);
  ...
  const acceptGameHandler = async (acceptedGameId: number, stake: string): Promise<void> => {
    ...
    const acceptGameResult = await gameApi.acceptGame(await getArbiter(), BigNumber.from(acceptedGameId), stake);
    ...
    if (lastMessage.messageType === 'ISignedGameMove') {
      ...
      const setNewGameState = async () => {
        try {
          ...
          const nextGameState = gameState.makeNewGameStateFromSignedMove(signedMove, isValid, isOpponentMove, previouseMove);
  ...
  useInterval(async () => {
    ...
    let players: [string, string] = await gameApi.getPlayers(await getArbiterRead(), BigNumber.from(gameId));
